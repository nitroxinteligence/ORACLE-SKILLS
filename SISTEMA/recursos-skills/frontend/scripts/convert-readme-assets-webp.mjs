// Modified for Oracle distribution: input asset directory is explicit. See signed manifest for provenance.
import fs from "fs";
import path from "path";
import sharp from "sharp";

const sourceAssets = process.env.ORACLE_README_ASSETS_DIR;
if (!sourceAssets) throw new Error("Set ORACLE_README_ASSETS_DIR to the original README asset directory before running this maintainer script.");
...
async function emilBadgeToWebp() {
  const src =
    path.join(sourceAssets, "c4f8c4a7-2566-4644-b752-b652e0c103f5.png");
...
async function sponsorLogoToWebp() {
  const jfif = path.join(sourceAssets, "6b610a0c-8889-49fc-9684-e172d7172ea0.jfif");
